...
The model appears to be useful. The f-statistic and the p-value of 000004058 suggests that the model is statistically significant. The multiple R-squared percentage indicates a high variability in wins, suggesting a good fit. The median, despite some variation, shows that the model is generally centered around the actual data points. The residuals are pretty balanced.
...
Points scored has the most impact on wins, while yard gains do not seem impactful at all, as it has the lowest score.
...
I would use this method to compare how different/multiple independent variables (which could include points scored, takeaways, etc.) affect the dependent variable (such as wins or losses).
