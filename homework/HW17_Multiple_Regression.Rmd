...
md_turnout |>
  ...
  labs(title = "Relationship between predictors and voter turnout",
       y = "Voter turnout (%)")
```
The correlation matrix shows that the strongest correlation for turnout is the percentage of unemployment (negative correlation) and the percentage of white residents (positive correlation). Of the scatterplots, percent unemployed, percent with bachelor's or higher and median age, seem to show the most linear correlation.
...
```
I think this model is useful. Firstly, the model accounts for 80% of the variance in turnout [after accounting for the variables]. Secondly, the f-statistic of 16.55 with a p-value of 2.99e-06, which is less than .05, means that the overall model is statistically significant. The model also has a low residual standard of error: 0.021. 

Some variables are not significant: unemployment, bachelors or higher and population. These coefficients had p-values over .05, meaning they are not statistically significant.
...
```{r}
# Model 2
model2 <- lm(turnout ~ median_age + pct_white + income + pct_unemployed, data = md_turnout)
summary(model2)
...
# Model 4
model4 <- lm(turnout ~ pct_white + income + median_age + pct_bachelors_higher, data = md_turnout)
summary(model4)
...
ggplot(model_comparison, aes(x = reorder(Model, Adjusted_R_squared), y = Adjusted_R_squared)) +
  ...
  labs(title = "Model Comparison by Adjusted R-squared",
       x = "Model")
```
All of the models do pretty well. However, Model 2 is the most significant. It has an adjusted R-square of .82, which is higher than the full model's adjusted R-square. This model (#2) included the three variables that were statistically significant and also pct_unemployed, which wasn't statistically signficant but it's p-value was not as high as the other variables (population and education).
...
To effectively cover an election or voter participation story, I could explore how different demographic and socioeconomic factors influence voter turnout across the state counties.
