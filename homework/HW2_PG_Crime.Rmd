...
```{r}
june_pct_change <- pg_crime_average |> mutate(june_change = (Jun-May)/May*100)
july_pct_change <- june_pct_change |> mutate(july_change = (Jul-Jun)/Jun*100) 
summer_crime_averages <- july_pct_change |> mutate(august_change = (Aug-Jul)/Jul*100)
```
...
The data shows a lot of decrease in crime during the summer in general, but in the summer the month with the highest increase rate and lowest decrease rate is June. It has higher rates in seven incidents. Of the three summer months, July was second with high rates in six incidents. August came third with high rates in five incidents. Something worth noting is that data showed no change from May to June in "auto stolen and recovered", "B&E commercial" and "robbery vehicle"; from June to July in "robbery residential"; and from July to August in "assault shooting", "homicide" and "robbery residential". A pattern I see is that the larger jumps of crime have been from May to June with up to a 75% increase in "assault with weapon" and incidents doubling in "robbery other" and "robbery residential".
